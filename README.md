...
### 1. [01-hello-world](file:///d:/ASL/remain-Go/go-languge/01-hello-world/main.go)
...
### 2. [02-variables-and-types](file:///d:/ASL/remain-Go/go-languge/02-variables-and-types/main.go)
...
### 3. [03-control-flow](file:///d:/ASL/remain-Go/go-languge/03-control-flow/main.go)
...
### 4. [04-slices-and-maps](file:///d:/ASL/remain-Go/go-languge/04-slices-and-maps/main.go)
...
### 5. [05-functions-and-pointers](file:///d:/ASL/remain-Go/go-languge/05-functions-and-pointers/main.go)
...
### 6. [06-structs-and-methods](file:///d:/ASL/remain-Go/go-languge/06-structs-and-methods/main.go)
...
### 7. [07-interfaces](file:///d:/ASL/remain-Go/go-languge/07-interfaces/main.go)
...
### 8. [08-error-handling](file:///d:/ASL/remain-Go/go-languge/08-error-handling/main.go)
...
### 9. [09-concurrency](file:///d:/ASL/remain-Go/go-languge/09-concurrency/main.go)
...
### 10. [10-testing](file:///d:/ASL/remain-Go/go-languge/10-testing/mathutil_test.go)
...
- **Run**:
  ```bash
  # Run all unit tests
  go test -v ./10-testing

  # Run benchmarks
  go test -v -bench="." ./10-testing
  ```

### 11. [11-mini-project-cli](file:///d:/ASL/remain-Go/go-languge/11-mini-project-cli/main.go)
...
### 12. [12-interfaces-advanced](file:///d:/ASL/remain-Go/go-languge/12-interfaces-advanced/main.go)
- **Concepts**: Interface contracts, Dependency Injection (DI) / Strategy Pattern, decoupled payment processing (`Razorpay`, `Stripe`, `PayPal`, `MockPaymentGateway`).
- **Run**:
  ```bash
  go run ./12-interfaces-advanced
  ```

### 13. [13-enums-and-custom-types](file:///d:/ASL/remain-Go/go-languge/13-enums-and-custom-types/main.go)
- **Concepts**: String enums, integer enums with `iota`, implementing `fmt.Stringer` interface, custom type method receivers.
- **Run**:
  ```bash
  go run ./13-enums-and-custom-types
  ```

### 14. [14-generics](file:///d:/ASL/remain-Go/go-languge/14-generics/main.go)
- **Concepts**: Go 1.18+ Generics, type parameters `[T any]`, `[T comparable]`, custom union constraints `[T Number]`, generic Stack data structure.
- **Run**:
  ```bash
  go run ./14-generics
  ```

### 15. [15-goroutines-and-waitgroups](file:///d:/ASL/remain-Go/go-languge/15-goroutines-and-waitgroups/main.go)
- **Concepts**: Goroutine scheduling, anonymous concurrent functions, synchronizing with `sync.WaitGroup` (`Add`, `Done`, `Wait`), loop variable capture safety.
- **Run**:
  ```bash
  go run ./15-goroutines-and-waitgroups
  ```

### 16. [16-channels-and-select](file:///d:/ASL/remain-Go/go-languge/16-channels-and-select/main.go)
- **Concepts**: Unbuffered channels, buffered channels, channel directionality (`chan<-`, `<-chan`), closing channels, ranging over channels, `select` multiplexing with timeouts and non-blocking `default`.
- **Run**:
  ```bash
  go run ./16-channels-and-select
  ```

### 17. [17-mutex-and-synchronization](file:///d:/ASL/remain-Go/go-languge/17-mutex-and-synchronization/main.go)
- **Concepts**: Preventing race conditions, `sync.Mutex` (`Lock`/`Unlock`), `sync.RWMutex` (multiple readers, single writer), atomic operations with `sync/atomic`.
- **Run**:
  ```bash
  go run ./17-mutex-and-synchronization
  ```

### 18. [18-file-handling](file:///d:/ASL/remain-Go/go-languge/18-file-handling/main.go)
- **Concepts**: File creation, writing bytes/strings, reading with `os.ReadFile` and `bufio.Scanner`, inspecting `os.Stat` metadata, directory listing with `os.ReadDir`, stream copying with `io.Copy`, and `os.Remove`.
- **Run**:
  ```bash
  go run ./18-file-handling
  ```

### 19. [19-packages-and-modules](file:///d:/ASL/remain-Go/go-languge/19-packages-and-modules/main.go)
- **Concepts**: Module structure, package separation (`auth`, `user`), Exported (Capitalized/Public) vs Unexported (lowercase/Private) identifiers, struct fields, constructor functions.
- **Run**:
  ```bash
  go run ./19-packages-and-modules
  ```
